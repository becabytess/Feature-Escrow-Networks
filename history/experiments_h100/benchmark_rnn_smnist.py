# ==============================================================================
# 2-Layer RNN Sweep on Sequential MNIST (sMNIST) Downsampled to 400 Steps (20x20)
...
# --- 1. CONFIGURATION ---
MODES = ["plain_rnn", "resnet_rnn", "fen_rnn", "fen_rotational_rnn"]
SEEDS = [100, 200]
EPOCHS = 15
BATCH_SIZE = 128
LR = 1e-3
...
print(f"Device: {device} | 2-Layer RNN sMNIST Sweep | Layers: {NUM_LAYERS} | Seq Len: {SEQ_LEN}")
...
class DeepRNN(nn.Module):
    def __init__(self, in_dim, hidden_dim, num_layers, mode):
        ...
        # Use native PyTorch nn.RNN layers for high speed execution
        self.cells = nn.ModuleList([
            nn.RNN(hidden_dim, hidden_dim, nonlinearity='tanh', batch_first=True) for _ in range(num_layers)
        ])
        
        self.has_escrow = mode in ["fen_rnn", "fen_rotational_rnn"]
    ...
    def forward(self, x):
        ...
        for l in range(self.num_layers):
            ...
            if self.mode == "plain_rnn":
                h = out
            elif self.mode == "resnet_rnn":
                h = out + h
            elif self.mode == "fen_rnn":
                f_raw = out + h
                g = torch.sigmoid(self.gate[l](f_raw))
                D = g * f_raw
                h = f_raw - D
                E = E + self.escrow_proj[l](D)
            elif self.mode == "fen_rotational_rnn":
                f_raw = out + h
                g = torch.sigmoid(self.gate[l](f_raw))
                D = g * f_raw
                h = f_raw - D
...
def choose_hidden(mode, target_params):
    best_h = 8
    best_diff = float('inf')
    for h in range(8, 512, 4):
        model = DeepRNN(1, h, NUM_LAYERS, mode)
...
def save_history_to_csv(history, filepath="experiments_h100/rnn_smnist_history.csv"):
...
def train_one(seed, mode, train_loader, val_loader):
    ...
    model = DeepRNN(1, h_dim, NUM_LAYERS, mode).to(device)
    params = count_params(model)
    
    print("\n" + "=" * 80, flush=True)
    print(f"RNN SMNIST | MODE={mode.upper()} | SEED={seed} | Width={h_dim} | Params={params:,}", flush=True)
    print("=" * 80, flush=True)
...
if __name__ == "__main__":
    ...
    for mode in MODES:
        h_dim = choose_hidden(mode, TARGET_PARAMS)
        model_test = DeepRNN(1, h_dim, NUM_LAYERS, mode)
    ...
    print("\n" + "#" * 80, flush=True)
    print(f"STARTING RNN SWEEP | TARGET_PARAMS = {TARGET_PARAMS:,} | EPOCHS = {EPOCHS}", flush=True)
    print("#" * 80, flush=True)
